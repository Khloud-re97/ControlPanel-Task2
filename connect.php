...
if(isset($_POST["save"])){
	...
	if(isset($_POST["forward"])){
		...
		if($conn->connect_error){
		  ...
		  $stmt = $conn->prepare("INSERT INTO `controlpanel` (`forward`) Values (1)");
		  $stmt->execute();
		  echo "Forward";
		}
	}
	if(isset($_POST["left"])){
		...
		if($conn->connect_error){
			...
			$stmt = $conn->prepare("INSERT INTO `controlpanel` (`To_left`) Values (1)");
			$stmt->execute();
			echo "Left";
		}
	}
	if(isset($_POST["stop"])){
		...
		if($conn->connect_error){
			...
			$stmt = $conn->prepare("INSERT INTO `controlpanel` (`stop`) Values (1)");
			$stmt->execute();
			echo "Stop";
		}
	}
	if(isset($_POST["right"])){
		...
		if($conn->connect_error){
			...
			$stmt = $conn->prepare("INSERT INTO `controlpanel` (`To_right`) Values (1)");
			$stmt->execute();
			echo "Right";
		}
	}	
	if(isset($_POST["backward"])){
		...
		if($conn->connect_error){
			...
			$stmt = $conn->prepare("INSERT INTO `controlpanel` (`backward`) Values (1)");
			$stmt->execute();
			echo "Backward";
		}
	}	